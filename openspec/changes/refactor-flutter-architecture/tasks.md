...
- [ ] 1.1 Create `lib/domain/value_objects/result.dart` with `Result<T, E>` sealed class, `Success`, `Failure`, and `when` pattern matching. **Verify:** `flutter test test/unit/value_objects/result_test.dart` passes.
